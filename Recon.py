...
def serviceName(ports, fila=None):
    ...
    with open("data.json", 'r') as json_file:
        service = re.findall("rq/\w*/(\w*)R", json_file.read(), re.I | re.DOTALL) # Le indico que lo que viene después de un rq/ es el nombre del servicio
...
def force_scan_all (direction, paquetes=None):
    ...
    tcpdump(prog=conf.prog.tshark, args=["-T", "json", "-f udp portrange 7400-7450", "-c", paquetes, "-w", 'data.pcap'], dump=True, quiet=True)  # -c quiere decir cuátos paquetes capturar.


    [hosts, output] = network_scan(direction)
    t.add_row(['Hosts', '¿Running in VM?', 'MAC', '¿Running node ROS2?', 'Instances of ROS2']).set_cols_valign(["m", "m", "m", "m", "m"]).set_cols_align(["c", "c", "c", "c", "c"])
    for host in hosts:
        row = []
        row.append(host)
        #VM
        info_vm = output.get('scan').get(host)
        string_vm = str(info_vm)

        if 'virtualbox' in string_vm.lower():
            row.append("Yes")
        else:
            row.append("No")

        #MAC
        info_mac = info_vm.get('addresses').get('mac')
        row.append(info_mac)
        ...
        num_nodes = len(matches) // 2 # Por cada listener o talker se abren 2 puertos por lo que capturo todos los puertos abiertos del 7400-7500 y lo divido entre 2
        list_nodes = []
        ...
        if num_nodes != 0: # A partir de aquí decimos que esta máquina ejecuta al menos un nodo ROS2
            ...
            row.append("Yes")
            tNode = Texttable(max_width=0)

            for i in range(num_nodes):
                tNode.add_row(['Node', 'Topic Suscribed/Publishing', 'Services', 'DDS/RTPS vendor', 'Security']).set_cols_valign(["m", "m", "m", "m", "m"]).set_cols_align(["c", "c", "c", "c", "c"])
                rowNode = []
                rowNode.append(list_nodes[i])

                #Topics
                topicName(list_nodes[i], rowNode)

                #Services
                serviceName(list_nodes[i], rowNode)

                #DDS/RTPS vendor
                DDS_vendor(list_nodes[i], rowNode)

                #Seguridad
                checkSeg(list_nodes[i], rowNode)

                tNode.add_row(rowNode)

            row.append(tNode.draw())
        else:
            row.append("No")
            row.append("-")
...
def ros2_scan(direction, paquetes=None):
    t = Texttable(max_width=0)
    ...
    [hosts, output] = network_scan(direction)
    t.add_row(['Hosts', 'Instances of ROS2']).set_cols_valign(["m", "m"]).set_cols_align(["c", "c"])
    for host in hosts:
        # Nodo Ros2
        info_node = nm.scan(hosts=host, arguments='-p 7410-7440 -sU -n -T2 --open', sudo=True)
        string_node = str(info_node.get('scan'))
        ...
        matches = re.findall("(\d{4})", string_node)
        num_nodes = len(matches) // 2  # Por cada listener o talker se abren 2 puertos por lo que capturo todos los puiertos abiertos del 7400-7500 y lo divido entre 2
        list_nodes = []
...
#######################################################
def force_scan_all_grepeable(direction, paquetes=None):
    if paquetes is None:
        session = PromptSession()
    ...
    for host in hosts:
        ...
        if num_nodes != 0:  # A partir de aquí decimos que esta máquina ejecuta al menos un nodo ROS2
            text[host] = {"¿Running in VM?" : vm, "MAC" : info_mac, "¿Running node ROS2?" : "Yes",  "Instances of ROS2" : []}
            ...
            for i in range(num_nodes):
                ...
                text.get(host).get("Instances of ROS2").append({"Node": list_nodes[i], "Topic Suscribed/Publishing": topics, "Services" : services, "DDS/RTPS vendor": vendor, "Security": seg})
        else:
            text[host] = {"¿Running in VM?" : vm, "MAC" : info_mac, "¿Running node ROS2?" : "No",  "Instances of ROS2" : []}
    ...
    for host in hosts:
        ...
        if num_nodes != 0:  # A partir de aquí decimos que esta máquina ejecuta al menos un nodo ROS2
            ...
            for i in range(num_nodes):
                ...
                text.get(host).get("Instances ROS2").append({"Node" : list_nodes[i], "Topic Suscribed/Publishing" : topics, "Services" : services, "DDS/RTPS vendor" : vendor, "Security" : seg})
